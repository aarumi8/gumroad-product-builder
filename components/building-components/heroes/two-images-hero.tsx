...
export function TwoImagesHero({ id, initialData, isPreview }: TwoImagesHeroProps) {
  ...
  const defaultContent = {
    ...
    images: initialData?.images || ["", ""] // Ensure we always have an array of two empty strings
  }

  // Use content from context if available, otherwise use default
  const content = currentComponent?.content || defaultContent
  const [editedContent, setEditedContent] = useState<typeof defaultContent>(content)
  ...
  // Get the current images array safely
        ...
        <div className="grid md:grid-cols-2 gap-8 relative z-10">
          {currentImages.map((imageUrl, index) => (
            <div key={index} className="relative">
              <div 
                className={`aspect-[4/3] rounded-2xl overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 shadow-xl ${isEditing && !isPreview ? 'cursor-pointer' : ''}`}
